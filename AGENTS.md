...
Before ANY tool call, query graphify_applystate first.
Call mcp_graphify_applystate_query_graph even if the task is small.
...
4. Call `mcp_graphify_applystate_query_graph(<next step context only>)`
...
4. Call `mcp_graphify_applystate_query_graph()` for full codebase context
